...
rm -rf build/
rm -f ../for_gerefi/wideband_image.h
...
# Copy the bin without the bootloader (the image consumed by gerEfi has no bootloader on it)
...
cp ${DELIVER_DIR}/wideband_image_with_bl.bin ../for_gerefi/
cp ${DELIVER_DIR}/wideband_image.h ../for_gerefi/
